...
def process_metadata(response: requests.Response):
    ...
    model = ResponseModel()
    model.services = {
        id: {"name": service["name"], "deal": deals.get(service["deal"], "Unknown Deal")}
...
def process_response(response: requests.Response, model: ResponseModel):
    ...
    json = response.json()
    if "data" not in json:
        raise ValueError(f"Response JSON does not contain 'data' key: {json}")

    for item in json["data"]:
        try:
            minutes: float = item["attributes"]["time"]
            service = model.get_service(item["relationships"]["service"]["data"]["id"])
        except KeyError as exc:
            raise ValueError(f"Malformed response item: {item}") from exc
...
async def hours(api_key: str, org_id: str, person_id: str = Header(None)):
    ...
    Args:
        api_key (str): _description_. The API key to authorise requests.
        org_id (str): _description_. The organisation ID to get hours from.
        person_id (str, optional): _description_. The person ID to get hours for.
    """
    if api_key is None:
    ...
    while response.status_code == 200 and "next" in response.links: # Response links isn't a thing.
